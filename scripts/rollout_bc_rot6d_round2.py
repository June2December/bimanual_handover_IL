...
# round1 rollout evaluation logs
SAVE_DIR = "/home/june/bimanual_handover_IL/data/rollout_bc_rot6d_round2_eval"
...
print("[INFO] round1 model loaded")
...
def run_one_episode(ep_idx):
    ...
    csv_path = os.path.join(
        SAVE_DIR,
        f"round1_rollout_phase4_zonly_ori_fixed_ep{ep_idx:03d}_{int(time.time())}.csv"
    )
    ...
    try:
        for step in range(MAX_STEPS):
            ...
            should_stop, stop_reason = check_episode_termination(
                step=step,
                max_steps=MAX_STEPS,
            ...
            csv_writer.writerow([
                ...
                *info["phase_onehot"].tolist(),
                ...
                stop_reason if should_stop else "",
                1 if success else 0,
            ])

            if should_stop:
                fail_reason = stop_reason
    ...
    print(f"[SAVE] episode log saved to {csv_path}")
    print(f"[RESULT] success={success}, reason={fail_reason}")

...
# Paths
# =========================
CSV_PATH = "/home/june/bimanual_handover_IL/data/merged/handover_merged_round2.csv"
MODEL_PATH = "/home/june/bimanual_handover_IL/checkpoints/bc_rot6d_round2/best.pt"
STATE_STATS_PATH = "/home/june/bimanual_handover_IL/checkpoints/bc_rot6d_round2/state_norm_stats.json"
ACTION_STATS_PATH = "/home/june/bimanual_handover_IL/checkpoints/bc_rot6d_round2/action_norm_stats.json"

OUT_DIR = "/home/june/bimanual_handover_IL/checkpoints/bc_rot6d_round2/eval"
os.makedirs(OUT_DIR, exist_ok=True)

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

TARGET_PHASES_1 = [4, 5, 6, 7, 8]
TARGET_PHASES_2 = [9, 10, 11]
...
def save_phase_summary_table(df_phase, target_phases, out_csv):
    rows = []
...
def make_phase_xyz_figure(df_phase, target_phases, out_png, title_prefix=""):
    n_rows = len(target_phases)
    fig, axes = plt.subplots(n_rows, 3, figsize=(14, 4.2 * n_rows))

    if n_rows == 1:
        axes = np.expand_dims(axes, axis=0)

    phase_to_row = {p: i for i, p in enumerate(target_phases)}
...
def main():
    ...
    # phase 4~8
    # =========================
    key_df_1 = result_df[result_df["phase"].isin(TARGET_PHASES_1)].copy()

    key_rows_csv_1 = os.path.join(OUT_DIR, "eval_rows_phase458.csv")
    key_df_1.to_csv(key_rows_csv_1, index=False)

    table1_csv = os.path.join(OUT_DIR, "table1_phase458_summary.csv")
    table1_df = save_phase_summary_table(key_df_1, TARGET_PHASES_1, table1_csv)

    figure1_png = os.path.join(OUT_DIR, "figure1_gt_vs_pred_xyz_phase458.png")
    make_phase_xyz_figure(key_df_1, TARGET_PHASES_1, figure1_png)

    # =========================
    # phase 9~11
    # =========================
    key_df_2 = result_df[result_df["phase"].isin(TARGET_PHASES_2)].copy()

    key_rows_csv_2 = os.path.join(OUT_DIR, "eval_rows_phase911.csv")
    key_df_2.to_csv(key_rows_csv_2, index=False)

    table2_csv = os.path.join(OUT_DIR, "table2_phase911_summary.csv")
    table2_df = save_phase_summary_table(key_df_2, TARGET_PHASES_2, table2_csv)

    figure2_png = os.path.join(OUT_DIR, "figure2_gt_vs_pred_xyz_phase911.png")
    make_phase_xyz_figure(key_df_2, TARGET_PHASES_2, figure2_png)

    # Console summary
    print("\n===== TABLE 1: OFFLINE PREDICTION SUMMARY (PHASE 4/5/6/7/8) =====")
    print(table1_df.to_string(index=False))

    print("\n===== TABLE 2: OFFLINE PREDICTION SUMMARY (PHASE 9/10/11) =====")
    print(table2_df.to_string(index=False))

    print("\n[INFO] saved files:")
    print(key_rows_csv_1)
    print(table1_csv)
    print(figure1_png)
    print(key_rows_csv_2)
    print(table2_csv)
    print(figure2_png)

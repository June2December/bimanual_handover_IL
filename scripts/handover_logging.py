...
LOG_DIR = "/home/june/bimanual_handover_IL/data"
...
def handover_episode(csv_path, cyl_spawn_pos):
    ...
    try:
        for step in range(MAX_STEPS):
            ...
            action_right_delta = np.zeros(3, dtype=np.float64)

            # -------------------------
            # left arm control
            # -------------------------
            if phase in [0, 1, 2, 4, 5, 6, 9, 10]:
                # move phase: RMPFlow 사용
                ...
                # close phase: arm 자세 고정 + finger만 닫기
                ...
                # release phase: arm 자세 고정 + finger만 열기
                apply_arm_hold_with_grip(
                    robot=left_robot,

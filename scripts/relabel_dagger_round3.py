...
# =========================
ROLLOUT_DIR = Path(
    "/home/june/bimanual_handover_IL/data/rollout_bc_rot6d_round2_eval"
)

BASE_MERGED_PATH = Path(
    "/home/june/bimanual_handover_IL/data/merged/handover_merged_round2.csv"
)
...
# =========================
# round2 merged에서 phase별 target 계산
# left action pos median 사용
# grip은 binary 유지
# =========================
phase_target_map = {}
...
# =========================
# rollout csv 전부 모아서 phase 6/7/8만 추출
# =========================
dagger_parts = []
...
for path in rollout_paths:
    ...
    if len(sub) == 0:
        print(f"[WARN] no phase 6/7/8 rows in {path.name}")
        continue

    # 파일별 episode_id 분리
    sub["episode_id"] = f"dagger_round3_{path.stem}"
...
print(f"\n[INFO] total dagger rows (phase 6~8): {len(dagger_df)}")
...
# 저장
# =========================
relabel_path = SAVE_DIR / "dagger_round3_relabel.csv"
merged_path = SAVE_DIR / "handover_merged_round3.csv"
...
print(f"rows added  : {len(dagger_df)}")
